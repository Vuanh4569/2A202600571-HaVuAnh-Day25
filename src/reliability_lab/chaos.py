    ...
    for breaker in gateway.breakers.values():
        ...
        for transition in breaker.transition_log:
            if transition.get("to") == "open":
                open_ts = float(transition["ts"])
            elif transition.get("to") == "closed" and open_ts is not None:
                recovery_times.append((float(transition["ts"]) - open_ts) * 1000.0)
                open_ts = None
...
def run_scenario(config: LabConfig, queries: list[str], scenario: ScenarioConfig) -> RunMetrics:
    ...
    metrics.circuit_open_count = sum(
        1 for breaker in gateway.breakers.values() for transition in breaker.transition_log if transition.get("to") == "open"
    )
